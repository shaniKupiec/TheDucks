...
function MapComponent() {
  ...
  useEffect(() => {
    ...
    for (let index = 0; index < duckList.length; index++) {
      duckFeatures.push(createFeature(duckList[index]));
    }
    var duckLayer: VectorLayer<Vector<Geometry>> = createVectorLayer(duckFeatures);
    map.addLayer(duckLayer);
    // rabbits
    var rabbitFeatures: Feature[] = [];
    for (let index = 0; index < rabbitList.length; index++) {
      rabbitFeatures.push(createFeature(rabbitList[index]));
    }
    var rabbitLayer: VectorLayer<Vector<Geometry>> = createVectorLayer(rabbitFeatures);
